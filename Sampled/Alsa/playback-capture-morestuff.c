...
main (int argc, char *argv[])
{
  ...
  */
  ...
  /**************** stuff something into the output buffer ****************/
  
  printf("Writing silnce\n");
  ...
  while (1) {
    /*
      ...
      printf("Wiat fail capture: %s\n", snd_strerror(err));
      exit(0);
      }
      if ((can_read = snd_pcm_avail_update (capture_handle)) < 0) {
      if (can_read == -EPIPE) {
      ...
      printf("Can read %d frames\n", can_read);

      if ((err = snd_pcm_wait(playback_handle, 1000)) < 0) {
      printf("Wiat fail playback: %s\n", snd_strerror(err));
      exit(0);
      }
    */
    int nread;
    ...
    /*
      int can_deliver;
      if ((can_deliver = snd_pcm_avail_update (playback_handle)) < 0) {
      if (can_deliver == -EPIPE) {
      fprintf (stderr, "an xrun occured\n");
      continue;
      } else {
      fprintf (stderr, "unknown ALSA avail update return value (%d)\n", 
