/*
 *  Latency test program - modeified JN
 *
                  ...
                  const char *id)
{
  ...
  //err = snd_pcm_sw_params_set_start_threshold(handle, swparams, 4);
  if (err < 0) {
...
int setparams(snd_pcm_t *phandle, snd_pcm_t *chandle, int *bufsize)
{
 ...
 __set_it:
  ...
  fprintf(stderr, "p perdio size got %d\n", p_psize);
  if (p_psize > (unsigned int)*bufsize)
    *bufsize = p_psize;
  snd_pcm_hw_params_get_period_size(c_params, &c_psize, NULL);
  fprintf(stderr, "c perdio size got %d\n", p_psize);
  if (c_psize > (unsigned int)*bufsize)
    *bufsize = c_psize;
  snd_pcm_hw_params_get_period_time(p_params, &p_time, NULL);
  snd_pcm_hw_params_get_period_time(c_params, &c_time, NULL);
  if (p_time != c_time) {
    printf("p time %d, c time %d\n", p_time, c_time);
    goto __again;
  }
...
int main(int argc, char *argv[])
{
  ...
  print_pcm_state(phandle, "playback");
  print_pcm_state(chandle, "capture");
  ...
  if ((err = snd_pcm_start(chandle)) < 0) {
    printf("Go capture error: %s\n", snd_strerror(err));
    exit(0);
  }

  /*
    if ((err = snd_pcm_start(phandle)) < 0) {
    printf("Go playback error: %s\n", snd_strerror(err));
    exit(0);
    }
  */  
  print_pcm_state(phandle, "playback");
  ...
  while (1) {
    if ((err = snd_pcm_readi (chandle, buffer, latency)) != latency) {
      fprintf (stderr, "read from audio interface failed (%s) read %d\n",
    ...
    printf("copying\n");
    /*
      if ((err = snd_pcm_writei (phandle, buffer, latency)) != latency) {
