...
static void display_help() {
    fprintf(stderr, "Usage: record output_file");
}
...
void setInputDevice(const char *name) {
   OMX_CONFIG_BRCMAUDIOSOURCETYPE sSource;

   if (name && strlen(name) < sizeof(sSource.sName)) {
       setHeader(&sSource, sizeof(OMX_CONFIG_BRCMAUDIOSOURCETYPE));

       err = OMX_GetParameter(handle, OMX_IndexConfigBrcmAudioSource, &sSource);
       if (err != OMX_ErrorNone) {
	   fprintf(stderr, "Error on setting audio destination\n");
	   exit(1);
       }
       strcpy((char *)sSource.sName, name);
       
       err = OMX_SetParameter(handle, OMX_IndexConfigBrcmAudioSource, &sSource);
       if (err != OMX_ErrorNone) {
	   fprintf(stderr, "Error on setting audio destination\n");
	   exit(1);
       }
...
#ifdef RASPBERRY_PI
    char *componentName = "OMX.broadcom.audio_capture";
#else
...
# endif
    ...
    nPorts = ((OMX_PORT_PARAM_TYPE)param).nPorts;
    if (nPorts > 1) {
	fprintf(stderr, "Record device has more than one audio port\n");
	exit(1);
    }
	...
	fprintf(stderr, "Port is not an output port\n");
	exit(1);
    }
    ...
    /* CHANGE THIS */
    err = OMX_FillThisBuffer(handle, pEmptyBuffer);
    /*
    for (;;) {
