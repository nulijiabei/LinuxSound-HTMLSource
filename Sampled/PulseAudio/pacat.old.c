...
static int volume_is_set = FALSE;
...
static int sample_spec_set = FALSE;

static pa_channel_map channel_map;
static int channel_map_set = FALSE;
...
int main(int argc, char *argv[]) {
    ...
    if (!pa_proplist_contains(proplist, PA_PROP_MEDIA_NAME)) {
        ...
        if (!pa_proplist_contains(proplist, PA_PROP_MEDIA_NAME)) {
            printf("Failed to set media name.");
            goto quit;
        }
    }
    ...
    assert_se(pa_signal_init(mainloop_api) == 0);
