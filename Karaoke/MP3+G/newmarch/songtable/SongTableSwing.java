...
public class SongTableSwing extends JPanel {
	...
	if (args.length >= 1 && 
	    args[0].startsWith("-h")) {
	    System.err.println("Usage: java SongTableSwing [songdir|songfile] url-for-songstore");
	...
	try {
	    ...
	    */
	    ...
	    // explicitly convert to UTF8 as Windows stuffs it up
	    DataOutputStream dout = new DataOutputStream(socket.getOutputStream());
	    song.path += '\n';
  
	    byte[] bytes = song.path.getBytes("UTF8");
	    ...
	    */
	    ...
	    dout.write(song.path.getBytes("UTF8"));
	    dout.flush();
