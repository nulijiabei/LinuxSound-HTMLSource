...
ctl_lyric(int lyricid)
{
	...
	if ((lyric[0] == ME_KARAOKE_LYRIC))
	{
		...
		pipe_int_write(LYRIC_MESSAGE);
		pipe_string_write(lyric_buf);
	    }
	    else if(lyric[1] == '@')
	    {
		if(lyric[2] == 'L')
...
ctl_read(int32 *valp)
{
    ...
    ret = ctl_blocking_read(valp);
    printf("Called ctl read, val %d\n", ret);    
    return(ret);
#if 0
...
ctl_pass_playing_list(int number_of_files, char *list_of_files[])
{
	...
	if (command==RC_LOAD_FILE) {
	    ...
	    char *title = get_midi_title(file_to_play);
	    fprintf(stderr, "MIDI title is %s\n", title);
